...
- [x] Validado ponta-a-ponta no browser real (2026-07-10): fabrics_amiibo (kind `positive`) marcado no Tracker → staged no editor → "Write & download save" → reimport do arquivo gerado → item aparece com badge `sav`, contagem 19/29 → 20/29, total de itens 4.795 → 4.796. Mecanismo de escrita escalar confirmado funcional para qualquer stat/categoria `positive`/`bool`/`seed`, não só amiibo. Script de regressão em `scripts/spike-write-amiibo.mjs` (replica writesForItem+applyEdits+reimport sem precisar do browser).
- [x] Editor v2 — escrita estrutural no pouch (2026-07-12): materiais/itens-chave/armaduras viraram graváveis de verdade, não só o editor de flags escalares do v1. Achado-chave via inspeção binária do save real: os arrays String64 do pouch (materials `d96ebf12`, armor_inventory `754e8549`, key_items `22c6530a`) têm **capacidade fixa maior que o uso normal** (510/300/200 slots pra 251/136/38 tipos conhecidos), com os slots sobrando vazios e contíguos no final (nome="" + sentinela `0xFFFFFFFF` no array paralelo de estoque dos materiais). Escrever num slot vazio não move nenhum outro byte do arquivo nem precisa mexer em ponteiro — documentado e validado byte-a-byte em `scripts/spike-write-pouch.mjs` (grant de material nunca obtido + bump de estoque + grant de armadura + grant de item-chave, zero diffs fora dos ranges tocados). Implementado em `saveWriter.ts`: `ArrayWrite`/`arrayWritesForItem` (materials/key_items via slot vazio; armor_inventory/armor_upgraded localizam o slot já ocupado pela peça — via `baseId` compartilhado entre os dois stats — e sobrescrevem com o id do tier alvo, permitindo pular direto pro 4★ sem precisar da peça base primeiro). Quantidade de material é um mecanismo à parte (`materialQty` no store, `MaterialQtyEdit[]` no `buildEditPlan`) já que é inerentemente numérico, não um boolean de tracker. UI: `Inventory.tsx` — materiais ganharam input numérico inline por card; itens-chave e armaduras (posse da peça, não upgrade) ganharam toque-pra-marcar igual fabrics/amiibo, com trava visual (`locked`) pros itens já confirmados pelo save (bug corrigido na mesma sessão: cards confirmados pelo save continuavam clicáveis sem fazer nada — pareciam quebrados). Testado ponta-a-ponta no browser: material nunca obtido + qtd 50 → gravado → reimportado → 50 persistido, não mais staged; armadura nunca possuída → gravada só no tier 0★ → reimportada → aparece travada/confirmada, sem afetar a peça irmã "(lowered)" que já estava em 4★. Total de itens do save foi de 4.795 pra 4.798 (+3), batendo exato. Fora do v2: guid (bubbulfrogs/hudson/sage's will) e compendium (reverse) — não usam arrays do pouch, ficam pro v3 se pedido.
- [x] Tab de Inventário v1 (2026-07-10): nova página `Inventory.tsx` (`/inventory`, nav própria) com 3 abas — Materiais (251, quantidade real do pouch quando há save na sessão, `lib/inventory.ts::buildMaterials`), Itens-Chave (38, owned/not owned) e Armaduras (136, estrelas 0-4★ lidas do pouch por peça, `buildArmor`). Sem save na sessão cai pra owned/not owned via `manual`/`fromSave` persistidos (sem quantidade/estrelas) + aviso. Armas/escudos/arcos e receitas de comida ficam fora (dataset não tem esses itens como pouch navegável — ver nota antiga no HANDOFF). Validado no browser: 249/251 materiais com contagem real (Brightbloom Seed×999 etc.), 135/136 armaduras com ★★★★ corretas.
- [x] Inventário v2 — grid visual + edição real (2026-07-12): feedback do usuário na v1 foi "sheet stackada, não parece inventário, não dá pra editar aqui". Reescrito como grade de cards quadrados (ícone + badge de quantidade/estrelas, estilo pouch do jogo) em vez de lista, com barra de abas por ícone (`components/TypeIcon.tsx`). Sem asset de ícone por item no dataset (nem redistribuir textura do jogo seria correto) → classificador `lib/materialIcon.ts` por prefixo do `actorName` (fruta/cogumelo/planta/carne/peixe/inseto/minério/monstro/zonai/misc), 10 ícones desenhados no estilo de linha já usado no resto do app. Adicionadas abas **Fabrics** e **Fabrics (Amiibo)** com edição real por toque — únicas categorias do Inventário que o save format realmente grava hoje (kind `positive`, mesmo mecanismo validado ponta-a-ponta antes); toque marca `staged` (anel jade, "not written yet") e linka pra Save → Edit & export. Materiais/Itens-Chave/Armaduras continuam somente leitura (escrita de array do pouch é Editor v2, fora de escopo). Testado no browser: 251 ícones renderizando com variedade correta (Bokoblin Fang→monstro, Golden Apple→fruta, Diamond→minério etc.), toggle de amiibo gera staged change real refletida no editor.
...
- [x] IA hospedada via ai-proxy (2026-07-10, fora desta sessão): `VITE_GEMINI_API_KEY` foi removida do bundle — o provider default agora é `hosted` (Edge Function Supabase compartilhada da suíte). Requer `VITE_SUPABASE_URL` + `VITE_SUPABASE_ANON_KEY` no ambiente de build.
- [ ] **Pendente do usuário em PROD (2026-07-12)**: dois ajustes na dashboard da Vercel, nenhum dos dois eu posso fazer (root dir é infra do projeto; env var é credencial — ambos fora do que faço sozinho):
  1. **Build quebrado**: projeto ficou conectado ao GitHub com a raiz do repo como Root Directory, mas o app vive em `app/` → `vite: command not found`. Corrigir em Settings → Build and Deployment → Root Directory → `app`.
  2. **IA "vermelha" em prod**: faltam `VITE_SUPABASE_URL` (`https://ebaxejpvcvbifaviptas.supabase.co`) e `VITE_SUPABASE_ANON_KEY` (já em `app/.env.local`) nas env vars do projeto. Sem elas, `HOSTED_AI_AVAILABLE` fica falso e a companion cai pra "sem IA" (planos determinísticos continuam funcionando normalmente, só a narração/parsing de frase livre fica sem IA).
- [x] Ícones de materiais nos cards do Inventário (2026-07-12): não há ícone por item no dataset (nem seria correto redistribuir textura do jogo) — classificador por tipo em `lib/materialIcon.ts` (fruta/cogumelo/planta/carne/peixe/inseto/minério/monstro/zonai/misc) com ícones vetoriais próprios, ver F6.
...
### F7 — Inventário v3 e rota com teleporte (2026-08-13)
- [x] **Inventário v3 — pouch de verdade**: mostrava os 251 materiais do dataset ordenados por quantidade; o jogo mostra só o que você carrega, na ordem dele. Achado ao inspecionar o binário: o array do save está em ordem de **coleta** (BombFruit, FireFruit, LightBall_Large…), não de exibição — quem já está na ordem do jogo é o **dataset** (frutas → cogumelos → plantas → carnes → peixes → insetos → minérios → partes de monstro → zonai). Logo, a correção é parar de reordenar e filtrar pra `owned`, com toggle "só os que tenho / mostrando tudo" pro editor ainda alcançar os não-possuídos. UI virou grade de slots + painel de detalhe fixo à direita (nome, tipo, selo de origem, quantidade ou botão de conceder), com anel dourado no selecionado. Validado com o save demo: 249/251 materiais na ordem certa, 135 armaduras com estrelas, 19/29 tecidos amiibo.
- [x] **Planner v2 com teleporte** (`lib/routePlanner.ts`): o v1 tratava o mapa como TSP puro (uma linha, tudo a pé), o que é o modelo errado — teleporte pra santuário/torre/raiz liberado é grátis. O v2 agrupa os pendentes em volta das âncoras de teleporte que o save mostra como desbloqueadas e resolve um TSP pequeno por agrupamento (vizinho-mais-próximo + **2-opt**). Medido em `scripts/spike-route-compare.mjs` contra saves de vários estágios: **30% menos caminhada** no save de ~90% (pendentes espalhados), 1–4% no início de jogo (pendentes densos) — o ganho depende da dispersão, e o painel mostra a comparação em vez de prometer número fixo. Defeito pego na medição e corrigido: passe único descartava em silêncio o excedente de um agrupamento maior que o teto por perna.
- [x] **Painel de rota** (`components/RouteArtifact.tsx`): abre à direita do chat da Purah como artefato — tiles de número grande, rota traçada por cima da imagem real da camada (uma polilinha por perna, âncoras douradas, paradas numeradas), lista de pernas, e botão pro mapa cheio. Estrutura visual seguindo a referência de dashboard do usuário, na paleta do app.
- [x] `MapPage` parou de desenhar uma linha contínua entre paradas separadas por teleporte (sugeria caminhada inexistente) — quebra em trechos via `legStart` no `RouteStep`.
- [ ] **Port pro Switch (Atmosphere) — adiado por decisão do usuário em 2026-08-13.** Pesquisa feita, pra não refazer: **não existe caminho barato.** O web applet do libnx (`webOfflineCreate`) só carrega HTML de conteúdo **NCA instalado** (`OfflineHtmlPage`/`ApplicationLegalInformation`/`SystemDataPage`), **não do cartão SD** — então não dá pra simplesmente apontar o browser do Switch pra PWA buildada no SD. Os caminhos reais são: (a) **rewrite nativo `.nro`** com libnx + borealis (C++), que lê o `progress.sav` direto do SD e tem controle/touch nativos, mas é reescrever tudo; (b) **adaptar a PWA** pro browser do Switch (gamepad API + navegação espacial por foco + layout TV-safe), barato mas depende de abrir o browser via hack e não é "um app homebrew". **Distribuição**: o baseline é um `.nro` solto em `/switch/` no SD, publicado via GitHub Releases; a **Homebrew App Store** (fortheusers/hb-appstore) aceita submissão pelo site deles (`hb-app.store/submit-or-request`) e serve repos como JSON + zips hospedados estaticamente — dá pra auto-hospedar um repo próprio também, sem depender da loja principal.
